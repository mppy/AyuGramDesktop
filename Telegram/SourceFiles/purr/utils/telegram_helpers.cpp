...
#include "purr/ui/boxes/donate_info_box.h"
#include "purr/ui/toasts.h"
#include "purr/utils/rc_manager.h"
...
const auto regDateBotFallbackUsername = QString("purrgrambot");
	...
	return [=]
	{
		...
		if (isCustomBadge) {
			...
			text = custom.text.isEmpty()
					   ? (isExtera
							  ? tr::purr_DeveloperPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  tr::rich)
							  : tr::purr_SupporterPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  tr::rich))
			...
			text = peer->isUser()
					   ? tr::purr_DeveloperPopup(
						   ...
						   TextWithEntities{peer->name()},
						   tr::rich)
					   : tr::purr_OfficialResourcePopup(
						   tr::now,
						   lt_item,
						   TextWithEntities{peer->name()},
						   tr::rich);
		} else if (isSupporter) {
			text = tr::purr_SupporterPopup(
				tr::now,
				lt_item,
				TextWithEntities{peer->name()},
				tr::rich);
...
void MarkAsReadThread(not_null<Data::Thread*> thread) {
	...
	PurrWorker::markAsOnline(&thread->session());
}
					 ...
					 [=](Fn<void()> finish)
					 {
						 if (const auto channel = history->peer->asChannel()) {
							 return history->session().api().request(MTPchannels_ReadHistory(
								 ...
								 MTP_int(tillId)
							 )).done([=] { PurrWorker::markAsOnline(&history->session()); }).send();
						 }
						 ...
						 )).done([=](const MTPmessages_AffectedMessages &result)
						 {
							 ...
							 PurrWorker::markAsOnline(&history->session());
						 }).fail([=]
						 {
						 }).send();
					 });
...
void markReadAfterAction(not_null<History*> history) {
	const auto &ghost = PurrSettings::ghost(&history->session());
...
QString formatTTL(int time, bool isDoc) {
	if (time == 0x7FFFFFFF) {
		return isDoc ? tr::purr_OnePlayTTL(tr::now) : tr::purr_OneViewTTL(tr::now);
	}
...
void processMessageDelete(not_null<HistoryItem*> item) {
	if (!isMessageSavable(item)) {
		...
		item->setDeleted();
		PurrMessages::addDeletedMessage(item);
	}
}
	...
	)).done([=](const MTPmessages_BotResults &result)
	{
		...
		for (const auto &res : v) {
			...
			if (flag == "EXACT" || flag == "INTERPOLATED") {
				if (!isSelf) {
					resultText = tr::purr_CreationDateUserApproximately(
						tr::now,
						lt_item1,
					...
					resultText = tr::purr_CreationDateSelfApproximately(
						tr::now,
						lt_item,
						TextWithEntities{formattedDate},
						tr::rich
				...
				if (!isSelf) {
					resultText = tr::purr_CreationDateUserEarlier(
						tr::now,
						lt_item1,
						TextWithEntities{userName},
						lt_item2,
					...
					resultText = tr::purr_CreationDateSelfEarlier(
						tr::now,
						lt_item,
						TextWithEntities{formattedDate},
						tr::rich
				...
				if (!isSelf) {
					resultText = tr::purr_CreationDateUserLater(
						tr::now,
						lt_item1,
						TextWithEntities{userName},
						lt_item2,
					...
					resultText = tr::purr_CreationDateSelfLater(
						tr::now,
						lt_item,
						TextWithEntities{formattedDate},
						tr::rich
...
void getChannelJoinOrCreateDate(not_null<ChannelData*> channel, Fn<void(TextWithEntities)> callback) {
	...
	if (channel->inviteDate) {
		...
		result = tr::purr_CreationDateChat(
			tr::now,
			lt_item1,
			TextWithEntities{channel->name()},
			lt_item2,
...
void applyGhostScheduling(
		...
		int delaySeconds) {
	const auto &ghost = PurrSettings::ghost(session);
