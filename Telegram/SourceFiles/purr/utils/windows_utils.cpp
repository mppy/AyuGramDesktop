...
void processLegacy(const QString &iconPath) {
	...
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/PurrGram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/PurrGram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcuts = {
		path + u"PurrGram Desktop/PurrGram.lnk"_q,
		path + u"PurrGram/PurrGram.lnk"_q,
...
void reloadAppIconFromTaskBar() {
	const auto iconPath = PurrAssets::appIcoPath();
