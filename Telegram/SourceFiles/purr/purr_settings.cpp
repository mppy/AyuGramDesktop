...
QString getSettingsPath() {
	return cWorkingDir() + u"tdata/purr_settings.json"_q;
}
	...
	if (val) {
		if (const auto window = Core::App().activeWindow()) {
			if (const auto session = window->maybeSession()) {
				PurrWorker::markAsOnline(session);
			}
		}
	}
}
...
PurrSettings::PurrSettings()
: _appIcon(PurrAssets::DEFAULT_ICON)
, _editedMark(Core::IsAppLaunched() ? tr::lng_edited(tr::now) : QString("edited")) {
}

PurrSettings &PurrSettings::getInstance() {
	static PurrSettings instance;
	return instance;
}

void PurrSettings::load() {
	...
	try {
		...
		if (!p.contains("ghostModeSettings")) {
			...
			LOG(("PurrGramSettings: migrated ghost mode settings to per-account format"));
		}

		try {
			from_json(p, settings);
		} catch (...) {
			LOG(("PurrGramSettings: failed to parse settings file"));
		}

		// --- Schema migration ---
		// Apply overrides for settings whose defaults changed since the file's
		// stored version. Each migration block bumps a specific setting back to
		// its new default if the file predates the version that introduced it.
		const auto fileVersion = settings._settingsVersion.current();
		if (fileVersion < 1) {
			// v1 introduced these changed defaults:
			//   saveForBots: false -> true
			//   showHideMessageInContextMenu: Hidden/Modifier -> Visible
			//   showUserMessagesInContextMenu: Hidden/Modifier -> Visible
			//   showMessageDetailsInContextMenu: Hidden/Modifier -> Visible
			//   showRepeatMessageInContextMenu: Hidden/Modifier -> Visible
			settings._saveForBots = true;
			settings._showHideMessageInContextMenu = ContextMenuVisibility::Visible;
			settings._showUserMessagesInContextMenu = ContextMenuVisibility::Visible;
			settings._showMessageDetailsInContextMenu = ContextMenuVisibility::Visible;
			settings._showRepeatMessageInContextMenu = ContextMenuVisibility::Visible;
			LOG(("PurrGramSettings: migrated v%1 -> v%2 (saveForBots + context menu visibility)").arg(fileVersion).arg(PurrSettings::kCurrentSettingsVersion));
		}
		settings._settingsVersion = PurrSettings::kCurrentSettingsVersion;
	} catch (...) {
		LOG(("PurrGramSettings: failed to read settings file (not json-like)"));
	}
...
void PurrSettings::save() {
	...
	if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
		LOG(("PurrGramSettings: failed to open settings file for writing"));
		return;
	}
	const auto bytes = QByteArray::fromStdString(p.dump(4));
	if (file.write(bytes) != bytes.size()) {
		LOG(("PurrGramSettings: failed to write settings file"));
		return;
	}
	if (!file.commit()) {
		LOG(("PurrGramSettings: failed to commit settings file"));
	}
}

void PurrSettings::reset() {
	getInstance() = PurrSettings();
	save();
}

GhostModeAccountSettings &PurrSettings::ghost(not_null<Main::Session*> session) {
	return ghost(session->userId().bare);
}

GhostModeAccountSettings &PurrSettings::ghost(uint64 userId) {
	auto &settings = getInstance();
...
GhostModeAccountSettings &PurrSettings::ghost() {
	if (const auto window = Core::App().activeWindow()) {
		if (const auto session = window->maybeSession()) {
...
void PurrSettings::setUseGlobalGhostMode(bool val) {
	if (_useGlobalGhostMode.current() == val) return;
	_useGlobalGhostMode = val;
	save();
}

void PurrSettings::addShadowBan(int64 id) {
...
void PurrSettings::removeShadowBan(int64 id) {
...
void PurrSettings::validate() {
	PurrSettings defaults;
	auto modified = false;
	...
	validateRange(_avatarCorners, 0, PurrUiSettings::kMaxAvatarCorners, defaults._avatarCorners);
...
void PurrSettings::setTranslationProvider(TranslationProvider val) {
	...
	if (const auto manager = Purr::Translator::TranslateManager::currentInstance()) {
		manager->resetCache();
	}
	save();
}

void PurrSettings::setAdaptiveCoverColor(bool val) {
	if (_adaptiveCoverColor.current() == val) return;
	_adaptiveCoverColor = val;
	save();
}

void PurrSettings::setImproveLinkPreviews(bool val) {
	if (_improveLinkPreviews.current() == val) return;
	_improveLinkPreviews = val;
	save();
}

void PurrSettings::setCrashReporting(bool val) {
	if (_crashReporting.current() == val) return;
	_crashReporting = val;
	save();
}

void PurrSettings::setAvatarCorners(int val) {
	if (_avatarCorners.current() == val) return;
	_avatarCorners = val;
	PurrUiSettings::setAvatarCorners(val);
	repaintApp();
	save();
}

void PurrSettings::setSingleCornerRadius(bool val) {
...
void to_json(nlohmann::json &j, const PurrSettings &s) {
	...
	j = nlohmann::json{
		{"settingsVersion", s._settingsVersion.current()},
...
void from_json(const nlohmann::json &j, PurrSettings &s) {
	PurrSettings defaults;

	s._settingsVersion = j.value("settingsVersion", 0);
