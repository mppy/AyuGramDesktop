...
TextWithEntities ChangeSummaryText(const ApplyChanges &changes) {
	...
	if (!changes.newFilters.empty()) {
		AppendChangeSummary(
			result,
			tr::purr_FiltersSheetNewFilters(
				tr::now,
				lt_count,
	...
	if (!changes.removeFiltersById.empty()) {
		AppendChangeSummary(
			result,
			tr::purr_FiltersSheetRemovedFilters(
				tr::now,
				lt_count,
	...
	if (!changes.filtersOverrides.empty()) {
		AppendChangeSummary(
			result,
			tr::purr_FiltersSheetUpdatedFilters(
				tr::now,
				lt_count,
	...
	if (!changes.newExclusions.empty()) {
		AppendChangeSummary(
			result,
			tr::purr_FiltersSheetNewExclusions(
				tr::now,
				lt_count,
	...
	if (!changes.removeExclusions.empty()) {
		AppendChangeSummary(
			result,
			tr::purr_FiltersSheetRemovedExclusions(
				tr::now,
				lt_count,
	...
	if (!changes.peersToBeResolved.empty()) {
		AppendChangeSummary(
			result,
			tr::purr_FiltersSheetDialogsToResolve(
				tr::now,
				lt_count,
...
void FilterUtils::importFromLink(const QString &link) {
	if (link.isEmpty()) {
		Ui::Toast::Show(tr::purr_FiltersToastFailFetch(tr::now));
		return;
	}
	...
	connect(
		...
		[=]
		{
			...
			if (jsonString.isNull()) {
				...
				Ui::Toast::Show(tr::purr_FiltersToastFailImport(tr::now));
...
void FilterUtils::publishFilters() {
	...
	titlePart.setBody("PurrGram Filters");
	...
	connect(
		...
		[=]
		{
			...
			if (error == QNetworkReply::NoError && location.isValid()) {
				...
				Ui::Toast::Show(tr::purr_FiltersToastFailPublish(tr::now));
			}
			reply->deleteLater();
		});
}
...
void FilterUtils::importFromJson(const QByteArray &json) {
	...
	if (!HasChanges(changes)) {
		Ui::Toast::Show(tr::purr_FiltersToastFailNoChanges(tr::now));
	...
	auto box = Ui::MakeConfirmBox({
		...
		.confirmed = [=](Fn<void()> close) {
			...
			try {
				...
				Ui::Toast::Show(tr::purr_FiltersToastSuccess(tr::now));
			} catch (...) {
				LOG(("FilterUtils: Failed to apply import changes"));
				Ui::Toast::Show(tr::purr_FiltersToastFailImport(tr::now));
			}
		},
		.confirmText = tr::purr_FiltersMenuImport(),
		.title = tr::purr_FiltersSheetTitle(),
	});
...
QString FilterUtils::exportFilters() {
	...
	const auto filters = PurrDatabase::getAllRegexFilters();
	...
	const auto excl = PurrDatabase::getAllFiltersExclusions();
...
ApplyChanges FilterUtils::prepareChanges(const QJsonObject &root) {
	...
	const auto existingFilters = PurrDatabase::getAllRegexFilters();
	const auto existingExclusions = PurrDatabase::getAllFiltersExclusions();
...
void FilterUtils::applyChanges(const ApplyChanges &changes) {
	if (!changes.newFilters.empty()) {
		for (const auto &filter : changes.newFilters) {
			PurrDatabase::addRegexFilter(filter);
		}
	}

	if (!changes.removeFiltersById.empty()) {
		for (const auto &id : changes.removeFiltersById) {
			PurrDatabase::deleteExclusionsByFilterId(id);
			PurrDatabase::deleteFilter(id);
		}
	}

	if (!changes.filtersOverrides.empty()) {
		for (const auto &filter : changes.filtersOverrides) {
			PurrDatabase::updateRegexFilter(filter);
		}
	}

	if (!changes.newExclusions.empty()) {
		for (const auto &exclusion : changes.newExclusions) {
			PurrDatabase::addRegexExclusion(exclusion);
		}
	}

	if (!changes.removeExclusions.empty()) {
		for (const auto &exclusion : changes.removeExclusions) {
			PurrDatabase::deleteExclusion(exclusion.dialogId, exclusion.filterId);
		}
	}
