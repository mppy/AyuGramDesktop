...
namespace PurrFeatures::MessageShot {
...
bool ignoreRender(RenderPart part) {
	...
	const auto &s = PurrSettings::getInstance().messageShotSettings();
	return isTakingShot()
...
void Make(not_null<QWidget*> box, const ShotConfig &config, const Fn<void(QImage&,bool)>& callback) {
	...
	const auto showBackground = PurrSettings::getInstance().messageShotSettings().showBackground();
...
std::shared_ptr<Ui::ChatStyle> BuildShotChatStyle(
	...
	const auto &shot = PurrSettings::getInstance().messageShotSettings();
...
void ShowMessageShotBox(
	...
	const PurrFeatures::MessageShot::ShotConfig config = {
		controller,
...
void Wrapper(not_null<HistoryView::ListWidget*> widget, HistoryItem *item, Fn<void()> clearSelected) {
	// PurrGram: if user clicked a single message without selection, use that item.
	WrapperImpl(
		widget,
		[item](const auto widget) {
			auto selected = widget->getSelectedIds();
			if (!selected.empty()) {
				return selected;
			}
			MessageIdsList result;
			if (item) {
