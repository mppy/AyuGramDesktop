...
void RegexEditBuilder(
	...
	if (filter) {
		box->setTitle(showToast ? tr::purr_RegexFiltersAdd() : tr::purr_RegexFiltersEdit());
		data = *filter;
	} else {
		box->setTitle(tr::purr_RegexFiltersAdd());
		data.enabled = true;
	...
	const auto caseInsensitive = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::purr_CaseInsensitiveExpression(tr::now),
	...
	const auto reversed = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::purr_ReversedExpression(tr::now),
			data.reversed,
			st::defaultBoxCheckbox),
		st::settingsCheckboxPadding);
	...
	auto saveAndClose = [=, id = data.id]
	{
		...
		crl::async([=]
		{
			PurrDatabase::addRegexFilter(newFilter);
			...
			crl::on_main([=]
			{
				...
				if (showToast) {
					auto config = Ui::Toast::Config{
						.text = tr::purr_RegexFilterBulletinText(
							tr::now,
							tr::rich),
						.adaptive = true,
					};
					if (dialogId.has_value()) {
						Purr::Ui::ShowToastWithAction(
							std::move(config),
							tr::purr_RegexFilterBulletinAction(tr::now),
							[=]() mutable {
								newFilter.dialogId = dialogId;

								PurrDatabase::updateRegexFilter(newFilter);
								FiltersCacheController::rebuildCache();
								FiltersCacheController::fireUpdate();
							});
					} else {
