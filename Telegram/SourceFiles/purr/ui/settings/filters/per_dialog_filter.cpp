...
void PerDialogFiltersListController::prepare() {
	...
	const auto exclusions = PurrDatabase::getAllFiltersExclusions();
	...
	for (const auto &[id, count] : countsByDialogIds) {
		...
		auto status = QString();
		if (count.filters > 0) {
			status += tr::purr_RegexFiltersAmount(tr::now, lt_count, count.filters);
			if (count.exclusions > 0) {
				status += ", ";
			}
		}
		if (count.exclusions > 0) {
			status += tr::purr_RegexFiltersExcludedAmount(tr::now, lt_count, count.exclusions);
		}
...
void PerDialogFiltersListController::rowClicked(not_null<PeerListRow*> peer) {
	...
	if (shadowBan) {
		...
		_contextMenu->addAction(
			...
			[=]
			{
				if (PurrSettings::getInstance().isShadowBanned(did)) {
					PurrSettings::getInstance().removeShadowBan(did);
				} else {
					PurrSettings::getInstance().addShadowBan(did);
				}
			},
			&st::menuIconDelete);
	...
	_controller->showSettings(PurrFiltersList::Id());
}
