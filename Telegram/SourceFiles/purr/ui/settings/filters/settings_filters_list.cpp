...
#include "purr/features/filters/filters_utils.h"
...
#include "purr/ui/settings/filters/per_dialog_filter.h"
...
rpl::producer<QString> PurrFiltersList::title() {
	if (shadowBan) {
		return tr::purr_FiltersShadowBan();
	}
	if (!dialogId.has_value()) {
		return tr::purr_RegexFiltersShared();
	}
	...
	if (from) {
		...
		res = tr::purr_RegexFiltersHeader(tr::now) + " (" + QString::number(did) + ")";
	}

	return rpl::single(res);
}

PurrFiltersList::PurrFiltersList(
	QWidget *parent,
...
void PurrFiltersList::checkBeforeClose(Fn<void()> close) {
	_controller->showExclude = true;
	_controller->shadowBan = false;
	close();
}

void PurrFiltersList::addNewFilter(const RegexFilter &filter, bool exclusion) {
	...
	auto defaultClickHandler = [=, dialogId = dialogId]() mutable
	{
		...
		_contextMenu->addAction(
			...
			[=]
			{
				...
				PurrDatabase::updateRegexFilter(*state);
		...
		_contextMenu->addAction(
			...
			[=]
			{
				PurrDatabase::deleteFilter(state->id);
				PurrDatabase::deleteExclusionsByFilterId(state->id);
				FiltersCacheController::rebuildCache();
				FiltersCacheController::fireUpdate();
			},
			&st::menuIconDelete);
	...
	auto exclusionsClickHandler = [=, controller = _controller, dialogId = dialogId]() mutable
	{
				...
				└── class Ui::ScrollArea
					└── class QWidget
						└── class Ui::PaddingWrap<class Ui::RpWidget>
							└── class Settings::PurrFiltersList
		 */
		...
		PurrDatabase::addRegexExclusion(newExclusion);
		FiltersCacheController::rebuildCache();
		FiltersCacheController::fireUpdate();
	...
	auto deleteExclusionsClickHandler = [=, this]() mutable
	{
		...
		_contextMenu->addAction(
			...
			[=, this]
			{
				...
				PurrDatabase::deleteExclusion(dialogId.value(), state->id);
				FiltersCacheController::rebuildCache();
				FiltersCacheController::fireUpdate();
			},
			&st::menuIconDelete);
...
void PurrFiltersList::initializeSharedFilters(
	not_null<Ui::VerticalLayout*> container) {
	if (dialogId.has_value() && _controller->showExclude.has_value() && _controller->showExclude.value()) {
		filters = PurrDatabase::getByDialogId(dialogId.value());
		exclusions = PurrDatabase::getExcludedByDialogId(dialogId.value());
	} else {
		filters = PurrDatabase::getShared();

		// remove shared filters that already excluded for that peer exclusion
		if (dialogId.has_value() && _controller->showExclude.has_value() && !_controller->showExclude.value()) {
			const auto excludedForDialogId = PurrDatabase::getExcludedByDialogId(dialogId.value());
	...
	if (!filters.empty()) {
		AddSkip(container);
		filtersTitle = AddSubsectionTitle(container, tr::purr_RegexFiltersHeader());
	...
	if (!exclusions.empty()) {
		...
		excludedTitle = AddSubsectionTitle(container, tr::purr_RegexFiltersExcluded());
	...
	if (filters.empty() && exclusions.empty()) {
		Ui::AddDividerText(container, tr::purr_RegexFiltersListEmpty());
	}
}

void PurrFiltersList::initializeShadowBan(not_null<Ui::VerticalLayout*> container) {
	...
	if (PurrSettings::getInstance().shadowBanIds().size() > 0) {
		AddSkip(container);
...
void PurrFiltersList::setupContent(not_null<Window::SessionController*> controller) {
	if (shadowBan) {
