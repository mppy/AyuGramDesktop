...
#include "purr/ui/boxes/donate_qr_box.h"
...
Asset getAsset(const QString &name) {
	...
	QString icon = QString(":/gui/icons/purr/donates/%1.svg").arg(normalized);
...
void BuildDonations(SectionBuilder &builder) {
	builder.add([](const BuildContext &ctx) {
		v::match(ctx, [&](const WidgetContext &wctx) {
			...
			AddSubsectionTitle(container, tr::purr_SupportHeader());
			AddDonate(
				AddButtonWithIcon(
					container,
			...
			AddDividerText(container,
				tr::purr_SupportDescription2(
					lt_item,
					rpl::single(
						Ui::Text::Link(tr::purr_SupportDescription1(tr::now), QString("tg://support"))
					),
					tr::marked));
		}, [&](const SearchContext &sctx) {
			sctx.entries->push_back({
				.id = u"purr/donate"_q,
				.title = tr::purr_SupportHeader(tr::now),
...
void BuildCrashReporting(SectionBuilder &builder, PurrSectionBuilder &purr) {
#ifndef TDESKTOP_DISABLE_AUTOUPDATE
	builder.addSkip();
	builder.addSubsectionTitle(tr::purr_CategoryOther());

	purr.addSettingToggle({
		.id = u"purr/crashReporting"_q,
		.altIds = { u"purr/crashlytics"_q },
		.title = tr::purr_CrashReporting(),
		.getter = &PurrSettings::crashReporting,
		.setter = &PurrSettings::setCrashReporting,
		.icon = { &st::menuIconReport },
	});
	builder.addSkip();
	builder.addDividerText(tr::purr_CrashReportingDescription());
#endif
}
...
void BuildOtherThings(SectionBuilder &builder) {
	...
	builder.addSkip();
	builder.addButton({
		.id = u"purr/registerUrlScheme"_q,
		.title = tr::purr_RegisterURLScheme(),
	...
	builder.addButton({
		.id = u"purr/resetSettings"_q,
		.title = tr::purr_ResetSettings(),
		.icon = { &st::menuIconRestore },
		.onClick = [=] {
			controller->show(Ui::MakeConfirmBox({
				.text = tr::purr_ResetSettingsConfirmation(tr::rich),
...
const auto kMeta = BuildHelper({
	...
	BuildCrashReporting(builder, purr);
	BuildOtherThings(builder);
});

} // namespace

rpl::producer<QString> PurrOther::title() {
	return tr::purr_CategoryOther();
}
