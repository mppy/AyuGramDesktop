...
namespace Settings::PurrBuilder {
namespace {
...
PurrSectionBuilder::PurrSectionBuilder(Builder::SectionBuilder &builder)
: _builder(builder) {
}

Builder::SectionBuilder &PurrSectionBuilder::base() {
	return _builder;
}

Ui::SettingsButton *PurrSectionBuilder::addSettingToggle(
		SettingToggleArgs &&args) {
	auto getter = [g = args.getter] {
		return (PurrSettings::getInstance().*g)();
	};
	auto setter = [s = args.setter](bool v) {
		(PurrSettings::getInstance().*s)(v);
	};
	return addToggle({
		.id = std::move(args.id),
...
Ui::SettingsButton *PurrSectionBuilder::addToggle(ToggleArgs &&args) {
...
Fn<void()> PurrSectionBuilder::addCollapsibleToggle(
...
void PurrSectionBuilder::addChooseButton(ChooseButtonArgs &&args) {
...
void PurrSectionBuilder::addSlider(SliderArgs &&args) {
	auto id = std::move(args.id);
	auto altIds = std::move(args.altIds);
	auto keywords = std::move(args.keywords);
	auto resolvedTitle = ResolveTitle(rpl::duplicate(args.title));
...
void PurrSectionBuilder::addBetaBadge(not_null<Ui::SettingsButton*> button) {
	AddBetaBadge(button);
}

void PurrSectionBuilder::addSectionDivider() {
	_builder.addSkip();
	_builder.addDivider();
	_builder.addSkip();
}
