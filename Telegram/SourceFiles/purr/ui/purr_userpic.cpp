...
bool ShouldOverrideShape(Ui::PeerUserpicShape shape) {
	...
	case Shape::Forum:
		return PurrSettings::getInstance().singleCornerRadius();
	}
	return false;
}

int ComputeRadius(int pixelSize) {
	const auto corners = PurrUiSettings::getAvatarCorners();
	if (corners >= PurrUiSettings::kMaxAvatarCorners) return pixelSize / 2;
	if (corners <= 0) return 0;
	return int(double(corners) / PurrUiSettings::kMaxAvatarCorners * pixelSize / 2.0);
}

double ComputeRadiusF(double size) {
	const auto corners = PurrUiSettings::getAvatarCorners();
	if (corners >= PurrUiSettings::kMaxAvatarCorners) return size / 2.0;
	if (corners <= 0) return 0.0;
	return double(corners) / PurrUiSettings::kMaxAvatarCorners * size / 2.0;
}

bool IsCircle() {
	return PurrUiSettings::getAvatarCorners() >= PurrUiSettings::kMaxAvatarCorners;
}

uint8 PackedState() {
	return uint8(PurrUiSettings::getAvatarCorners() & 0x1F)
		| (PurrSettings::getInstance().singleCornerRadius() ? 0x20 : 0);
}

void PaintShape(QPainter &p, int x, int y, int size) {
	const auto corners = PurrUiSettings::getAvatarCorners();
	if (corners >= PurrUiSettings::kMaxAvatarCorners) {
		p.drawEllipse(x, y, size, size);
	} else if (corners <= 0) {
		p.drawRect(x, y, size, size);
	} else {
		const auto r = double(corners) / PurrUiSettings::kMaxAvatarCorners * size / 2.0;
...
void PaintShape(QPainter &p, const QRectF &rect) {
	...
	if (corners >= PurrUiSettings::kMaxAvatarCorners) {
		...
		const auto r = double(corners) / PurrUiSettings::kMaxAvatarCorners
...
QPointF OnlineBadgePosition(int photoSize, double badgeSize, double stroke) {
	const auto corners = PurrUiSettings::getAvatarCorners();
	const auto r = double(corners) / PurrUiSettings::kMaxAvatarCorners * photoSize / 2.0;
