...
#include "purr/purr_state.h"
#include "purr/data/messages_storage.h"
#include "purr/features/filters/filters_controller.h"
#include "purr/features/forward/purr_forward.h"
#include "purr/ui/context_menu/menu_item_subtext.h"
#include "purr/ui/message_history/history_section.h"
#include "purr/ui/settings/filters/edit_filter.h"
#include "purr/ui/settings/filters/settings_filters_list.h"
#include "purr/utils/qt_key_modifiers_extended.h"
...
Fn<void()> ClearDeletedMessagesHandler(not_null<Window::SessionController*> controller, not_null<PeerData*> peer, ID topicId) {
	return [=] {
		controller->show(Ui::MakeConfirmBox({
			.text = tr::purr_ClearDeletedMessagesText(tr::now),
			.confirmed = [=](Fn<void()> &&close) {
				...
				PurrMessages::clearDeletedMessages(peer, topicId);
				for (const auto item : items) {
					item->destroy();
				}
				close();
			},
			.confirmText = tr::purr_ClearDeletedMessagesActionText(tr::now),
...
Fn<void()> DeleteMyMessagesHandler(not_null<Window::SessionController*> controller, not_null<PeerData*> peer) {
	return [=]
	{
		if (!controller->showFrozenError()) {
			controller->show(Ui::MakeConfirmBox({
				.text = tr::purr_DeleteOwnMessagesConfirmation(tr::now),
				.confirmed =
				[=](Fn<void()> &&close)
				{
...
void AddPurrGramActions(PeerData *peerData,
							   Data::Thread *thread,
	...
	addCallback(Window::PeerMenuCallback::Args{
		...
		.fillSubmenu = [=](not_null<Ui::PopupMenu*> menu) {
			...
			if (showFilters) {
				addAction(
					tr::purr_ViewFiltersMenuText(tr::now),
					[=]
					{
						...
						sessionController->showSettings(Settings::PurrFiltersList::Id());
					},
			...
			if (filteredToggleShown) {
				addAction(
					*filteredToggleShown
						? tr::purr_HideFilteredMessagesMenuText(tr::now)
						: tr::purr_ShowFilteredMessagesMenuText(tr::now),
					[=]
					{
			...
			if (saveDeletedMessages) {
				addAction(
					tr::purr_ViewDeletedMenuText(tr::now),
					[=]
					{
				...
				addAction({
					.text = tr::purr_ClearDeletedMenuText(tr::now),
...
void AddJumpToBeginningAction(PeerData *peerData,
	...
	addCallback(
		tr::purr_JumpToBeginning(tr::now),
		[=]
		{
			if (user) {
...
void AddShadowBanAction(PeerData *peerData,
	...
	const auto shadowBanned = PurrSettings::getInstance().isShadowBanned(realId);
	const auto toggleShadowBan = [=]
	{
		if (shadowBanned) {
			PurrSettings::getInstance().removeShadowBan(realId);
		} else {
			PurrSettings::getInstance().addShadowBan(realId);
		}
	};

	addCallback({
		.text = (shadowBanned
					 ? tr::purr_FiltersQuickUnshadowBan(tr::now)
					 : tr::purr_FiltersQuickShadowBan(tr::now)),
...
void AddDeleteOwnMessagesAction(PeerData *peerData,
	...
	addCallback(
		tr::purr_DeleteOwnMessages(tr::now),
...
void AddHistoryAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	const auto has = PurrMessages::hasRevisions(item);
	if (!has) {
		return;
	}

	menu->addAction(
		tr::purr_EditsHistoryMenuText(tr::now),
		[=]
		{
...
void AddHideMessageAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::purr_ContextHideMessage(tr::now),
		[=]()
		{
...
void AddUserMessagesAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	if (item->history()->peer->isChat() || item->history()->peer->isMegagroup()) {
		menu->addAction(
			tr::purr_UserMessagesMenuText(tr::now),
			[=]
			{
...
void AddMessageDetailsAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	callback(Window::PeerMenuCallback::Args{
		.text = tr::purr_MessageDetailsPC(tr::now),
		.handler = nullptr,
		...
		.fillSubmenu = [&](not_null<Ui::PopupMenu*> menu2)
		{
			if (hasAnyPostField) {
				if (!messageViews.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::purr_MessageDetailsViewsPC(tr::now),
						messageViews
					));
				}
				...
				if (!messageForwards.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::purr_MessageDetailsSharesPC(tr::now),
						messageForwards
					));
				}
			...
			menu2->addAction(Ui::ContextActionWithSubText(
				menu2->menu(),
				st::menuIconSchedule,
				tr::purr_MessageDetailsDatePC(tr::now),
			...
			if (view && view->displayedEditDate()) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconEdit,
					tr::purr_MessageDetailsEditedDatePC(tr::now),
			...
			if (isForwarded) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconTTL,
					tr::purr_MessageDetailsForwardedDatePC(tr::now),
			...
			if (media && hasAnyMediaField) {
				...
				if (!mediaSize.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconDownload,
						tr::purr_MessageDetailsFileSizePC(tr::now),
						mediaSize
					));
				}
				...
				if (!mediaMime.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconShowAll,
						tr::purr_MessageDetailsMimeTypePC(tr::now),
						mime.name()
					));
				}
				...
				if (!mediaName.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::purrEditsHistoryIcon,
						tr::purr_MessageDetailsFileNamePC(tr::now),
						shortified,
						[=]
						{
				...
				if (!mediaResolution.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconStats,
						tr::purr_MessageDetailsResolutionPC(tr::now),
						mediaResolution
					));
				}
				...
				if (!mediaDC.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconBoosts,
						tr::purr_MessageDetailsDatacenterPC(tr::now),
						mediaDC
					));
				}
	...
	menu->addAction(
		...
		[=]
		{
			...
			if (useNoQuote) {
				...
				if (PurrForward::isFullPurrForwardNeeded(currentItem)) {
					crl::async([=]
					{
						PurrForward::forwardMessages(session, action, false, resolvedDraft);
					});
				} else if (PurrForward::isPurrForwardNeeded(currentItem)) {
					crl::async([=]
					{
						PurrForward::intelligentForward(session, action, resolvedDraft);
					});
				} else {
...
void AddReadUntilAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	const auto &ghost = PurrSettings::ghost(&item->history()->session());
	if (ghost.sendReadMessages()) {
		return;
	}

	menu->addAction(
		tr::purr_ReadUntilMenuText(tr::now),
		[=]
		{
			readHistory(item);
...
void AddBurnAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::purr_ExpireMediaContextMenuText(tr::now),
		[=]
		{
			const auto ids = MTP_vector<MTPint>(1, MTP_int(item->id));
...
void AddCreateFilterAction(not_null<Ui::PopupMenu*> menu,
	...
	menu->addAction(
		tr::purr_RegexFilterQuickAdd(tr::now),
		[=]
		{
			RegexFilter filter;
