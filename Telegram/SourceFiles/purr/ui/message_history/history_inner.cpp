...
#include "purr/ui/message_history/history_inner.h"
...
void InnerWidget::preloadMore(Direction direction) {
	...
	crl::async([=] {
		std::vector<PurrMessageBase> messages;
		if (item) { // viewing edited history
			messages = PurrMessages::getEditedMessages(item, minId, maxId, perPage);
		} else { // viewing deleted messages
			messages = PurrMessages::getDeletedMessages(peer, topicId, minId, maxId, perPage, searchQuery);
		}
...
void InnerWidget::addMessages(Direction direction, const std::vector<PurrMessageBase> &messages) {
