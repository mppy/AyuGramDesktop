...
#include "purr/ui/boxes/plugin_info_box.h"
...
void FillPluginInfoBox(
...

	{
		...
		const auto versionPrefix = tr::purr_PluginVersion(tr::now)
			+ u" "_q
			+ metadata.version;
...

	{
		...
		auto descText = hasDescription
			...
			: TextWithEntities{tr::purr_PluginNoDescription(tr::now)};
		if (hasDescription) {
