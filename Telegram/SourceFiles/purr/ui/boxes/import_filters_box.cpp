...
void FillImportFiltersBox(not_null<Ui::GenericBox*> box, bool import) {
	...
	box->setTitle(import ? tr::purr_FiltersMenuImport() : tr::purr_FiltersMenuExport());
	...
	addOption(false, import ? tr::purr_FiltersImportClipboard(tr::now) : tr::purr_FiltersExportClipboard(tr::now));
	addOption(true, import ? tr::purr_FiltersImportURL(tr::now) : tr::purr_FiltersExportURL(tr::now));
	...
	const auto actionButton = box->addButton(
		import ? tr::purr_FiltersMenuImport() : tr::purr_FiltersMenuExport(),
		[=]
		{
