...
void Content::setupContent(
	...
	const auto current = PurrUiSettings::getMonoFont();
...
PurrUi::FontSelectorBox::FontSelectorBox(QWidget *, Window::SessionController *controller, Fn<void(QString font)> hook)
	: _controller(controller), _hook(hook) {
}

void PurrUi::FontSelectorBox::prepare() {
	...
	addLeftButton(tr::purr_BoxActionReset(),
				  [=]
				  {
					  _hook(qs(""));
	...
	setTitle(tr::purr_CustomizeFontTitle());
	...
	const auto fonts = PurrUi::PrepareFonts();
	const auto inner = setInnerWidget(
		object_ptr<PurrUi::Content>(this, fonts),
		st::boxScroll,
		topContainer->height());
...
void PurrUi::FontSelectorBox::setupTop(not_null<Ui::VerticalLayout*> container) {
	if (!_controller) {
		return;
	}
}

void PurrUi::FontSelectorBox::keyPressEvent(QKeyEvent *e) {
...
int PurrUi::FontSelectorBox::rowsInPage() const {
	return std::max(height() / PurrUi::Rows::DefaultRowHeight(), 1);
}

void PurrUi::FontSelectorBox::setInnerFocus() {
	_setInnerFocus();
}

base::binary_guard
PurrUi::FontSelectorBox::Show(Window::SessionController *controller, const Fn<void(QString font)> hook) {
