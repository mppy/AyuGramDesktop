...
#include "lang_auto.h"
#include "purr/features/message_shot/message_shot_theme_state.h"
...
void ThemeSelectorBox::setupContent() {
	...
	setTitle(tr::purr_MessageShotThemeSelectTitle());
	...
	PurrFeatures::MessageShot::themeChosen(
	) | rpl::on_next(
	...
	PurrFeatures::MessageShot::paletteChosen(
	) | rpl::on_next([=](const auto &palette)
							 {
								 const auto type = PurrFeatures::MessageShot::getSelectedFromDefault();
								 const auto name = (type != Window::Theme::EmbeddedType(-1))
									 ? PurrFeatures::MessageShot::embeddedThemeDisplayName(type)
									 : tr::purr_MessageShotThemeDefault(tr::now);
								 _themeNames.fire(QString(name));
								 _selectedPalette = palette;
							 },
							 lifetime());

	addButton(tr::purr_MessageShotThemeApply(),
			  [=]
			  {
