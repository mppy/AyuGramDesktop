...
QImage MakeSupportLogo() {
	...
	auto svg = QSvgRenderer(QString(":/gui/icons/purr/donates/support_logo.svg"));
	auto image = QImage(
...
void FillDonateInfoBox(not_null<Ui::GenericBox*> box, not_null<Window::SessionController*> controller) {
	...
	box->verticalLayout()->add(
		object_ptr<Ui::FlatLabel>(
			box->verticalLayout(),
			tr::purr_SupportBoxHeader(tr::bold),
			st::boxTitle),
		st::boxRowPadding,
		style::al_top);
	...
	const auto str = tr::purr_SupportBoxMakeDonationInfo(
		tr::now,
		lt_amount1,
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::purr_SupportBoxMakeDonationHeader(tr::now),
		str,
		&st::menuIconEarn,
		emojiHelper.context()));
	...
	const TextWithEntities proofText = tr::purr_SupportBoxSendProofInfo(
		tr::now,
		lt_item,
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::purr_SupportBoxSendProofHeader(tr::now),
		proofText,
		&st::menuIconPhoto,
		Core::TextContext({
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::purr_SupportBoxReceiveBadgeHeader(tr::now),
		TextWithEntities{
			tr::purr_SupportBoxReceiveBadgeInfo(tr::now)
		},
