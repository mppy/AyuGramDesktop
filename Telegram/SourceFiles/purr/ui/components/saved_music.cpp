...
#include "purr/ui/components/saved_music.h"

#include "purr/purr_settings.h"
#include "purr/ui/utils/color_utils.h"
#include "purr/ui/utils/itunes_search.h"
#include "purr/ui/utils/palette.h"
...
QRgb AdjustHsl(QRgb color, float luminance, float saturation = -1.0f) {
	auto hsl = Purr::Ui::ColorUtils::colorToHSL(color);
	...
	return Purr::Ui::ColorUtils::HSLToRGB(hsl);
}
...
QPixmap MakeNoCoverImage(const QSize &size) {
	...
	auto image = QImage(size, QImage::Format_ARGB32);
	{
		...
		auto svgIcon = QSvgRenderer(u":/gui/icons/purr/nocover.svg"_q);
...
std::optional<QRgb> ExtractColorFromCover(const QPixmap &cover) {
	const auto palette = Purr::Ui::Palette::from(cover).generate();
	...
	const auto contrast = Purr::Ui::ColorUtils::calculateContrast(whiteColor, extractedColor);
	...
	if (Purr::Ui::ColorUtils::calculateContrast(whiteColor, adjustedColor) < 3.0f) {
...
PurrMusicButton::PurrMusicButton(
	QWidget *parent,
...
PurrMusicButton::~PurrMusicButton() = default;

void PurrMusicButton::updateData(MusicButtonData data) {
...
void PurrMusicButton::downloadAndMakeCover(FullMsgId msgId) {
...
void PurrMusicButton::makeCover() {
	...
	crl::async(
		[=, mediaView = _mediaView, performerText = _performerText, titleText = _titleText, overrideBg = _overrideBg]()
		{
			...
			if (cover.noCover) {
				const auto pix = Purr::Ui::Itunes::FetchCover(performerText, titleText, size);
				if (!pix.isNull()) {
			...
			crl::on_main([weak, cover = std::move(cover), bgColor, overrideBg]() mutable
			{
				...
				const auto &settings2 = PurrSettings::getInstance();
...
void PurrMusicButton::paintEvent(QPaintEvent *e) {
	if (!_currentCover) {
		return;
	}
...
int PurrMusicButton::resizeGetHeight(int newWidth) {
