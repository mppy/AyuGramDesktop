...
const QVector<QString> icons{
	PurrAssets::DEFAULT_ICON,
	PurrAssets::ALT_ICON,
	PurrAssets::DISCORD_ICON,
	PurrAssets::SPOTIFY_ICON,
	PurrAssets::EXTERA_ICON,
	PurrAssets::NOTHING_ICON,
	PurrAssets::BARD_ICON,
	PurrAssets::YAPLUS_ICON,
	PurrAssets::WIN95_ICON,
	PurrAssets::CHIBI_ICON,
	PurrAssets::CHIBI2_ICON,
	PurrAssets::EXTERA2_ICON,
};
...
void applyIcon() {
	...
	Window::OverrideApplicationIcon(PurrAssets::currentAppLogo());
	...
	for (int row = 0; row < rows; row++) {
		...
		for (int i = 0; i < columns; i++) {
			...
			if (const auto cached = _cachedIcons.find(iconName); cached != _cachedIcons.end()) {
				...
				icon = _cachedIcons[iconName] = PurrAssets::loadPreview(iconName);
			}
			auto opacity = 0.0f;
			if (iconName == _wasSelected) {
				opacity = 1.0f - _animation.value(1.0f);
			} else if (iconName == PurrAssets::currentAppLogoName()) {
		...
		for (int i = 0; i < columns; i++) {
			...
			if (e->pos().x() >= x && e->pos().x() <= x + iconSize
				...
				if (settings.appIcon() != iconName) {
					...
					PurrSettings::getInstance().setAppIcon(iconName);
					changed = true;
					break;
				}
			}
