...
, _state(lifetime().make_state<State>()) {
	_state->bubbleRadius = PurrSettings::getInstance().messageBubbleRadius();
	...
	const auto purrgramUser = HistoryView::GenerateUser(
		history,
		u"PurrGram Releases"_q);
	...
	rpl::merge(
		PurrSettings::getInstance().replaceBottomInfoWithIconsChanges()
			| rpl::to_empty,
		PurrSettings::getInstance().deletedMarkChanges()
			| rpl::to_empty,
		PurrSettings::getInstance().editedMarkChanges()
			| rpl::to_empty,
		PurrSettings::getInstance().removeMessageTailChanges()
			| rpl::to_empty,
		PurrSettings::getInstance().hideFastShareChanges()
			| rpl::to_empty,
		PurrSettings::getInstance().simpleQuotesAndRepliesChanges()
			| rpl::to_empty,
		PurrSettings::getInstance().semiTransparentDeletedMessagesChanges()
			| rpl::to_empty
	) | rpl::on_next([=] {
		refresh();
	}, lifetime());
...
void MessagePreview::paintEvent(QPaintEvent *e) {
	...
	if (!PurrSettings::getInstance().hideFastShare()) {
