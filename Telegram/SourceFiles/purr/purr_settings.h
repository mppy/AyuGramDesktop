...
#include "purr/libs/json.hpp"
#include "purr/libs/json_ext.hpp"
...
class PurrSettings {
public:
	// Current settings schema version. Bump this when changing the default
	// value of an existing setting. load() will apply migration overrides
	// for settings whose defaults changed since the file's stored version.
	static constexpr int kCurrentSettingsVersion = 1;

	PurrSettings(const PurrSettings &) = delete;
	PurrSettings &operator=(const PurrSettings &) = delete;
	PurrSettings(PurrSettings &&) noexcept = default;
	PurrSettings &operator=(PurrSettings &&) noexcept = default;

	[[nodiscard]] static PurrSettings &getInstance();
	...
	[[nodiscard]] int settingsVersion() const { return _settingsVersion.current(); }
	...
	friend void to_json(nlohmann::json &j, const PurrSettings &s);
	friend void from_json(const nlohmann::json &j, PurrSettings &s);
...
private:
	...
	// Bump when changing defaults of existing settings.
	// Migration logic in load() applies overrides for settings whose
	// defaults changed since the file's version.
	rpl::variable<int> _settingsVersion = kCurrentSettingsVersion;

	rpl::variable<bool> _saveDeletedMessages = true;
	rpl::variable<bool> _saveMessagesHistory = true;
	rpl::variable<bool> _saveForBots = true;
	...
	rpl::variable<ContextMenuVisibility> _showHideMessageInContextMenu = ContextMenuVisibility::Visible;
	rpl::variable<ContextMenuVisibility> _showUserMessagesInContextMenu = ContextMenuVisibility::Visible;
	rpl::variable<ContextMenuVisibility> _showMessageDetailsInContextMenu = ContextMenuVisibility::Visible;
	rpl::variable<ContextMenuVisibility> _showRepeatMessageInContextMenu = ContextMenuVisibility::Visible;
