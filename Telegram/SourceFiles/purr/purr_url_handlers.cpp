...
#include "purr/purr_url_handlers.h"
...
bool ResolveUser(
	...
	searchUserById(
		...
		[=](const QString &title, PeerData *data)
		{
			...
			Ui::show(Ui::MakeInformBox(tr::purr_UserNotFoundMessage()));
		}
	);
...
struct ResolvedSetting {
	QString controlId;
	::Settings::Type section = ::Settings::PurrMain::Id();
};
...
bool HandlePurrSettings(
	...
	if (settingName.isEmpty()) {
		controller->showSettings(::Settings::PurrMain::Id());
	} else {
		const auto resolved = ResolveSetting(
			u"purr/"_q + settingName,
			&controller->session());
