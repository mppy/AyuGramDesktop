...
#include "purr/libs/sqlite/sqlite_orm.h"
	...
	for (int v = currentVersion + 1; v <= kLatestVersion; ++v) {
		if (migrations.contains(v)) {
			try {
				...
				PurrDatabase::moveCurrentDatabase();
...
namespace PurrDatabase {

void moveCurrentDatabase() {
	const auto time = base::unixtime::now();

	if (QFile::exists("./tdata/purrdata.db")) {
		QFile::rename("./tdata/purrdata.db", QString("./tdata/purrdata_%1.db").arg(time));
	}

	if (QFile::exists("./tdata/purrdata.db-shm")) {
		QFile::rename("./tdata/purrdata.db-shm", QString("./tdata/purrdata_%1.db-shm").arg(time));
	}

	if (QFile::exists("./tdata/purrdata.db-wal")) {
		QFile::rename("./tdata/purrdata.db-wal", QString("./tdata/purrdata_%1.db-wal").arg(time));
	}
}
