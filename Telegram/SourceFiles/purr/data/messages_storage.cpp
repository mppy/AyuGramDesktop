...
std::vector<PurrMessageBase> convertToBase(const std::vector<DerivedMessage> &messages) {
	std::vector<PurrMessageBase> based;
	based.reserve(messages.size());
	for (const auto &msg : messages) {
		based.push_back(static_cast<PurrMessageBase>(msg));
	}
	return based;
}

void map(not_null<HistoryItem*> item, PurrMessageBase &message) {
	...
	message.flags = PurrMapper::mapItemFlagsToMTPFlags(item);
	...
	auto serializedText = PurrMapper::serializeTextWithEntities(item);
...
void addEditedMessage(not_null<HistoryItem *> item) {
	...
	PurrDatabase::addEditedMessage(message);
}
...
std::vector<PurrMessageBase> getEditedMessages(not_null<HistoryItem*> item, ID minId, ID maxId, int totalLimit) {
	...
	return convertToBase(PurrDatabase::getEditedMessages(userId, dialogId, msgId, minId, maxId, totalLimit));
}
...
bool hasRevisions(not_null<HistoryItem*> item) {
	...
	return PurrDatabase::hasRevisions(userId, dialogId, msgId);
}
...
void addDeletedMessage(not_null<HistoryItem*> item) {
	...
	PurrDatabase::addDeletedMessage(message);
}

std::vector<PurrMessageBase>
getDeletedMessages(not_null<PeerData*> peer, ID topicId, ID minId, ID maxId, int totalLimit, const QString &searchQuery) {
	...
	return convertToBase(
		PurrDatabase::getDeletedMessages(userId, getDialogIdFromPeer(peer), topicId, minId, maxId, totalLimit, searchQuery.toStdString()));
}

bool hasDeletedMessages(not_null<PeerData*> peer, ID topicId) {
	const ID userId = peer->session().userId().bare & PeerId::kChatTypeMask;
	return PurrDatabase::hasDeletedMessages(userId, getDialogIdFromPeer(peer), topicId);
}

void clearDeletedMessages(not_null<PeerData*> peer, ID topicId) {
	const ID userId = peer->session().userId().bare & PeerId::kChatTypeMask;
	PurrDatabase::clearDeletedMessages(userId, getDialogIdFromPeer(peer), topicId);
}
