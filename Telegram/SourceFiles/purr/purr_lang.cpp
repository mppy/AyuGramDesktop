...
PurrLanguage *PurrLanguage::instance = nullptr;

PurrLanguage::PurrLanguage() = default;

void PurrLanguage::init() {
	if (!instance) instance = new PurrLanguage;
	instance->loadCachedLanguage();
}

PurrLanguage *PurrLanguage::currentInstance() {
	return instance;
}

QString PurrLanguage::getCacheDir() const {
	return cWorkingDir() + u"tdata/purr/languages/"_q;
}

QString PurrLanguage::getCachePath(const QString &langId) const {
	return getCacheDir() + langId + u".json"_q;
}

void PurrLanguage::loadCachedLanguage() {
	...
	if (file.open(QIODevice::ReadOnly)) {
		...
		if (error.error == QJsonParseError::NoError) {
			LOG(("Loading cached PurrGram language: %1").arg(finalLangPackId));
			applyLanguageJson(doc);
		}
	}
}

void PurrLanguage::saveCachedLanguage(const QByteArray &json, const QString &langId) {
	...
	if (file.open(QIODevice::WriteOnly)) {
		file.write(json);
		file.close();
		LOG(("Cached PurrGram language: %1").arg(langId));
	}
}

void PurrLanguage::fetchLanguage(const QString &id, const QString &baseId) {
	...
	// https://crowdin.com/project/purrgram/discussions/6
	QUrl url;
	if (!finalLangPackId.isEmpty() && !baseId.isEmpty() && !needFallback) {
		url.setUrl(qsl("https://cdn.jsdelivr.net/gh/PurrGram/Languages@l10n_main/values/langs/%1/Shared.json").arg(
			finalLangPackId));
	} else {
		url.setUrl(qsl("https://cdn.jsdelivr.net/gh/PurrGram/Languages@l10n_main/values/langs/%1/Shared.json").arg(
...
void PurrLanguage::fetchFinished() {
	if (!_chkReply) return;
	...
	if (statusCode == 404 && !langPackId.isEmpty() && !langPackBaseId.isEmpty() && !needFallback) {
		LOG(("PurrGram Language not found! Fallback to main language: %1...").arg(langPackBaseId));
		needFallback = true;
...
void PurrLanguage::fetchError(QNetworkReply::NetworkError e) {
	...
	if (e == QNetworkReply::NetworkError::ContentNotFoundError) {
		...
		if (!id.isEmpty() && !baseId.isEmpty() && !needFallback) {
			LOG(("PurrGram Language not found! Fallback to main language: %1...").arg(baseId));
			needFallback = true;
			_chkReply->disconnect();
			fetchLanguage("", baseId);
		} else {
			LOG(("PurrGram Language not found!"));
			_chkReply = nullptr;
		}
	}
}

void PurrLanguage::applyLanguageJson(QJsonDocument doc) {
	const auto json = doc.object();
	for (const QString &brokenKey : json.keys()) {
		auto key = qsl("purr_") + brokenKey;
